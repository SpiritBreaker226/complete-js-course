...
for (const [team, odd] of Object.entries(game.odds)) {
  const victoryOrDraw = game[team] ? `victory ${game[team]}` : `draw`;

  console.log(`Odd of ${victoryOrDraw}: ${odd}`);
}
...
// allplays cames from Coding Challenge #1
const scorers = {};
...
const normalizeAirportCode = airportCode =>
  airportCode.slice(0, 3).toUpperCase();

for (const flight of flights.split('+_')) {
  const [type, from, to, time] = flight.split(';');
  const delayedIcon = type.includes('Delayed') ? '🔴 ' : '';

  console.log(
    `${delayedIcon}${type
      .replaceAll('_', ' ')
      .trim()} from ${normalizeAirportCode(from)} to ${normalizeAirportCode(
      to
    )} (${time.replace(':', 'h')})`.padStart(46)
  );
}
